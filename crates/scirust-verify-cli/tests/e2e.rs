...
#[test]
fn sbom_evidence_is_generated_and_attached() {
    prebuild_fixtures();
    ...
    let out = cli()
        ...
        .unwrap();
    assert!(out.status.success());
    assert!(String::from_utf8_lossy(&out.stdout).contains("sbom_generated"));

    let run_dir = project.join(format!(".scirust-verify/runs/{}", latest_run(&project)));
    let spdx_path = run_dir.join("sbom/spdx.json");
    assert!(
        spdx_path.is_file(),
        "SPDX document must be attached to evidence"
    );
    let spdx: serde_json::Value =
        serde_json::from_slice(&std::fs::read(&spdx_path).unwrap()).expect("valid SPDX JSON");
    assert_eq!(
        spdx.get("spdxVersion").and_then(|v| v.as_str()),
        Some("SPDX-2.3")
    );
    assert!(spdx.get("packages").and_then(|v| v.as_array()).is_some());
}
...
#[test]
fn capsule_verification_pass_and_tamper() {
    prebuild_fixtures();
    let bundle = fixture("scicap-demo");

    // Clean bundle verifies.
    let out = cli()
        .args(["verify-capsule", bundle.to_str().unwrap()])
        .output()
        .unwrap();
    assert!(
        out.status.success(),
    ...
    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(stdout.contains("scicap_payloads_intact"), "{stdout}");
    assert!(
        stdout.contains("UNSUPPORTED"),
        "execution boundary must show: {stdout}"
    );

    // Tampered payload fails integrity.
    let payload = bundle.join("README.md");
    let original = std::fs::read_to_string(&payload).unwrap();
    std::fs::write(&payload, original.replace("hello", "HELLO")).unwrap();
    let out = cli()
        .args(["verify-capsule", bundle.to_str().unwrap()])
        .env("RUST_BACKTRACE", "0")
        .output()
...
#[test]
fn forge_envelope_ingest_pass_and_mutate() {
    use scirust_verify_artifacts::forge::CandidateEnvelopeV1;

    prebuild_fixtures();
    let store = tempfile_dir("forge-store");
    let envelope_path = store.join("envelope.json");

    // Build a valid envelope using the same fingerprint algorithm upstream
    // defines; the CLI must independently recompute and accept it.
    let json = r#"{
  ...
  "producer_candidate_id": null,
  "proposal_sha256": null,
  "schema_version": 1,
  "source_sha256": "1111222233334444555566667777888899990000aaaabbbbccccddddeeeeffff",
  "trial_seed": "42"
}"#;
    let env = CandidateEnvelopeV1::parse(&json.replace("PLACEHOLDER", &"a".repeat(64))).unwrap();
    let fp = env.computed_fingerprint();
    std::fs::write(&envelope_path, json.replace("PLACEHOLDER", &fp)).unwrap();
    ...
    // Any field mutation breaks the bound fingerprint -> FAIL exit 1.
    let mutated = json
        .replace("PLACEHOLDER", &fp)
        .replace("\"trial_seed\": \"42\"", "\"trial_seed\": \"43\"");
    std::fs::write(&envelope_path, mutated).unwrap();
    let out = cli()
        .args([
            "ingest-forge",
            envelope_path.to_str().unwrap(),
